...
int j=0;
//电压曲线的原始数据
void dealvobuffer(float* buffer,uint8_t line,float n,uint8_t state)
{
	if(j<buffersize-1)//数据没能填满屏幕时，直接添加新数据
	{
		buffer[j]=n;
		j++;
	}
	else	//数据已填满屏幕时，将旧数据向左移一位，去掉最旧的数据，添上最新数据
	{	
		for(int i=0;i<buffersize-2;i++)
			buffer[i]=buffer[i+1];		
		buffer[buffersize-2]=n;
	}
	if (state==2)
	{
//		printf("f = %f\r\n",n);
		for(int u=0;u<4;u++)	
			printf("add 1,%d,%d\xff\xff\xff",line,(int)n);
		if(line == 1)
			printf( "x0.val=%d\xff\xff\xff",(int)(n*100) );
		else if(line == 2)
			printf( "x1.val=%d\xff\xff\xff",(int)(n*100) );
	}
	
}
//温度曲线的原始数据
void dealtpbuffer(int* buffer,int n,uint8_t line,uint8_t state)
{
	if(j<buffersize-1)//数据没能填满屏幕时，直接添加新数据
	{
		buffer[j]=n;
	...
	{
		for(int i=0;i<buffersize-2;i++)//数据已填满屏幕时，将旧数据向左移一位，去掉最旧的数据，添上最新数据
			buffer[i]=buffer[i+1];
		buffer[buffersize-2]=n;
	}
	if (state==1)
	{
		for(int u=0;u<4;u++)
			printf("add 1,%d,%d\xff\xff\xff",line,n);
		if(line == 1)
			printf( "x0.val=%d\xff\xff\xff",n*100 );
	}
	

}

int tp_buffer0[buffersize];
int tp_buffer1[buffersize];
float vl_buffer0[buffersize];
float vl_buffer1[buffersize];
float vl_buffer2[buffersize];
#define BUF_SIZE 30     // 提取数值的缓冲区大小
void extract_numbers(char *payload,uint8_t state) 
{
	char buf[BUF_SIZE];
	int i, j,k1=0,k0=0;
	float f;
...
//	printf("payload = %s\r\n",payload);
	...
	{
		// 检查字符是否为数字、小数点或负号
		if ((isdigit(payload[i]) || payload[i] == '.' || payload[i] == '-')&&(payload[i+1] != ':')) 
		{
			// 将字符添加到缓冲区
			buf[j++] = payload[i];
			// 检查是否缓冲区已满或下一个字符为非数字相关字符
			if (j == BUF_SIZE - 1 || ((!(isdigit(payload[i + 1]) ))&& payload[i + 1] != '.'))
			{
				buf[j] = '\0';			
				if (is_float) 
				{
					...
					{	
						switch(k0)
						{
							case 0:{dealvobuffer(vl_buffer0,0,f, state);k0++;break;}						
							case 1:{dealvobuffer(vl_buffer1,1,f, state);k0++;break;}
							case 2:{dealvobuffer(vl_buffer2,2,f, state);k0++;break;}							
							default:break;
						}								
					// 如果转换成功，则为浮点数
//						printf("浮点数: %.2f\r\n", f);
				...
				else 
				{
					...
					if (n != 0) 
					{
						switch(k1)
						{
							case 0:{dealtpbuffer(tp_buffer0,n,0, state);k1++;break;}						
							case 1:{dealtpbuffer(tp_buffer1,n,1, state);k1++;break;}
							default:break;
						}
					// 如果转换成功，则为整数
//						printf("整数: %d\n", n);						
		...
		// 如果遇到小数点，设置标志位用于将缓冲区中的字符串转换为浮点数
		if (payload[i] == '.' && isdigit(payload[i + 1])) 
		{
			is_float = true;
		}
	}
...
void RadioIrqProcess( void )
{
   // if( IrqFired == true )
	 uint8_t i;
	 uint16_t tmp;
		tmp = (uint16_t)(HAL_GPIO_ReadPin(RADIO_DIO1_PORT,RADIO_DIO1_PIN));
    if(tmp== GPIO_PIN_SET)//if(GPIO_ReadInputDataBit(RADIO_DIO1_PORT, RADIO_DIO1_PIN)==Bit_SET)
    {
        IrqFired = false;
        ...
        if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
			...
			for(i=0;i<30;i++)
				sprintf(tempcbuff,"%c",RadioRxPayload[i]);
					
//					
//					for(i=0;i<30;i++)
//								printf("%c",RadioRxPayload[i]);
//					printf("\r\n");
					
			extract_numbers(RadioRxPayload,tjcstate);
